...
keys, and then pass those keys to lookup and binding operations.
...
thread's metadata; this pointer points to a ``Map`` from ``Integer`` to an
untyped payload. There is at most one scoped thread-local per runtime key in a
scope. Changing a thread-local binding is a three-step process. First, a new
thread-local scope is constructed out of the previous thread-local scope with
the binding inserted, updated, or deleted. Next, a special frame is pushed to
the stack that
contains the previous thread-local scope, so that when control returns past this
frame later, the previous scope is restored. Finally, the thread-local scope
field of the thread is set equal to the new scope.
...
The implementation currently used as the model for this proposal already has
